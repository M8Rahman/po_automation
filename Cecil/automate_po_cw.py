...
def run_navigation():
    print("\n--- Phase 1: Navigation ---")
    if not find_and_click('merchandising_btn.png', 'Merchandising Module'): return False
    if not find_and_click('merchandising_menu.png', 'Merchandising Menu'): return False
    # if not find_and_click('buyer_po.png', 'Buyer Purchase Order', wait=0.5): return False 
    if not find_and_click('buyer_po.png', 'Buyer Purchase Order'): return False 
    if not find_and_click('po_entry.png', 'Purchase Order Entry'): return False
    return True
...
def process_po_data(po_data):
    ...
    if not find_and_click('create_new_po.png', 'Create New PO'): return

    # 1. Buyer
    if not find_and_click('buyer_dropdown.png', 'Buyer Dropdown'): return
